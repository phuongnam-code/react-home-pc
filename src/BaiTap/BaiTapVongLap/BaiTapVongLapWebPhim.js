...
export default class BaiTapVongLapWebPhim extends Component {
	constructor(props) {
		super(props);
		this.state = {
			dsPhim: [
	...
	renderWebPhim = () => {
		return this.state.dsPhim.map((phim, index) => {
			return (
				<div className="card text-left col-4 my-4">
					<img className="card-img-top" src={phim.hinhAnh} width="100%" alt />
					<div className="card-body">
						<h6 className="card-title">{phim.tenPhim}</h6>
						<p className="card-text">
							<span>{phim.ngayKhoiChieu}</span>
							<br />
							<span>Đánh giá: {phim.danhGia}</span>
						</p>
						<p>
							Mô tả:
							{phim.moTa.length > 100 ? phim.moTa.substr(0, 110) + "..." : phim.moTa}
						</p>
					</div>
				</div>
			);
	...
	render() {
		return (
			<div className="container">
				<h1>Bài tập web phim</h1>
				<div className="row">{this.renderWebPhim()}</div>
			</div>
		);
	}
}
